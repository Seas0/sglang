...
| `wan_rmsnorm_silu` | Triton | close | dense `channels_last_3d` 5D (`stride(C) == 1`), Wan VAE channel-first RMSNorm + SiLU; `pre_bias` absorbs the producing conv's deferred bias with aten's rounding |
