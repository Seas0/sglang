...
- **VAE:** full-image decoding is the default. Tiling can change pixels near
  ...
  path with fused RMSNorm+SiLU and fused conv-bias epilogues for both encode
  and decode; on one RTX 4090 at 1024×1024 that took decode from 246 ms to
  154 ms for text-to-image and edit alike, with output at SSIM 0.9997 (t2i)
  and 0.9988 / 43 dB (edit) against the lossless default, which stays
  bit-exact.
